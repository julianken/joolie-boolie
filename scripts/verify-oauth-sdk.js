 ...
 *
 * Run from monorepo root: node scripts/verify-oauth-sdk.js
 */
...
try {
  ...
  console.error('❌ Error: Cannot import @supabase/supabase-js');
...
(async () => {
  ...
  try {
    ...
    if (result.error) {
      ...
      console.log(`   📝 Error response: ${result.error.message || result.error.error_description || JSON.stringify(result.error)}`);
